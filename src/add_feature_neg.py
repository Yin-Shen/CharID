...
train = pd.read_table(args.dirs+"/result/whole_negative.bed", header=None)
train.loc[-1]=['chrom1','start1','end1','chrom2','start2','end2']
train.index = train.index + 1
train = train.sort_index()
train.to_csv(args.dirs+"/result/whole_negative_new.bed",sep='\t',header=None, index=False)

train = pd.read_table(args.dirs+"/result/whole_negative_new.bed")
...
train['response'] = 0
train.to_csv(args.out+'_neg_train.txt',sep='\t', index=False)

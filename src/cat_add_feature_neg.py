...
signal_files = []
for file in files:
    if file[-15::] == '_neg_trains.txt':
        signal_files.append(file[int(len_outs)+1:-15])
train = pd.read_table(args.dirs+"/result/"+args.out+"_neg_train.txt")
train.drop('index',axis=1, inplace=True)
for signal in signal_files:
    train_new = pd.read_table(args.out+"_"+str(signal)+"_neg_trains.txt")
